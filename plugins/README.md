...
| [deal-context](./deal-context/) | Persistent per-deal workspaces that aggregate CRM data, conversations, web research, and Monday.com signals into a daily ranked action plan | **Commands:** `/deal-init`, `/deal-refresh`, `/deal-status` - Scaffold, refresh, and check deal workspaces<br>**Agent:** `account-analyst` - Refreshes one account's workspace and recommends its next step |
